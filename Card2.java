...
public class Card2 extends Compare
{
    ...
    public String getSelectedCardNumber() {
        if (selected != null && !selected.isEmpty()) {
            String cardImagePath = selected.peek();
            // Extracting card number from the file path
            String[] parts = cardImagePath.split(" ");
            String cardNumber = parts[1]; // Assuming the card number is the second part
            return cardNumber;
        } else {
            return null; // Handle the case when no card is selected
        }
    }
